...
### PRD Coverage
AUTH-1 through AUTH-8 (§2.2)

### Audit Status: PASS
- `initialize()` has try-catch — `loading` always resolves to `false`
- `fetchOrCreateProfile` handles race conditions (duplicate key → retry SELECT)
- `signInWithMagicLink` has try-catch — returns user-facing error on network failure
...
4. User edits display name inline in sidebar → `updateProfile()` → Supabase `.update()` → local state
5. User picks avatar from AvatarPicker → `updateProfile({ avatar_url: 'icon:cat' })` → persisted

### Files
- `src/stores/authStore.ts` — `updateProfile`, `fetchOrCreateProfile`
- `src/components/Layout.tsx` — inline display name editing, avatar picker trigger, friend code copy, share link
- `src/components/AvatarPicker.tsx` — icon grid modal
- `src/components/AvatarIcon.tsx` — renders icon/image/initial avatars

### Sub-flows
- **Edit display name**: click pencil → inline input → blur/Enter saves → `updateProfile({ display_name })`
- **Pick avatar**: click avatar → AvatarPicker modal → select icon → `updateProfile({ avatar_url: 'icon:id' })`
- **Copy friend code**: click copy button → `navigator.clipboard.writeText()` → checkmark feedback
- **Share profile link**: click share → copies URL with friend code query param

### PRD Coverage
PROF-1 through PROF-6 (§2.2). PROF-5 (custom image upload) is 🔲 Future.

### Audit Status: PASS
- `updateProfile` has no loading gate — failures are silent but non-blocking
...
2. User creates a category → auto-assigned color from palette → stored in `clofri-friend-categories`
3. User assigns a friend to a category → stored in `clofri-friend-assignments` (friendshipId → categoryId)
...
### Files
- `src/stores/categoryStore.ts` — `addCategory`, `removeCategory`, `renameCategory`, `assignFriend`, `getCategoryForFriend`
- `src/components/Friends.tsx` — category sections, filter dropdown, assignment UI

### Sub-flows
- **Create category**: enter name → `addCategory(name)` → auto-color → localStorage
- **Assign friend**: dropdown on friend card → `assignFriend(friendshipId, categoryId)` → localStorage
- **Delete category**: confirm dialog → `removeCategory(id)` → removes category + all its assignments
- **Filter by category**: dropdown at top → filters visible friends list

### PRD Coverage
CAT-1 through CAT-8 (§2.4). CAT-6 (cross-device sync) is 🔲 Future.

### Audit Status: PASS
- All operations are synchronous localStorage — no network calls, no loading states needed
- `loadCategories`/`loadAssignments` have try-catch for corrupted localStorage
- Data is local-only — lost on browser data clear (documented as known limitation)
...
### PRD Coverage
DM-1 through DM-14 (§2.5). DM-14 (scroll pagination) is 🔲 Future.
...
### PRD Coverage
GRP-1 through GRP-13 (§2.6), CHAT-1 through CHAT-15 (§2.7).
CHAT-9 (media), CHAT-10 (reactions), CHAT-11 (link previews) are 🔲 Future.
...
### Flow
1. **Sound notifications**: Web Audio API synthesized sounds — message pop + nudge buzz
...
4. **Cross-page DM alerts**: lobby channel `new_dm` broadcast → presenceStore handler → marks unread + plays sound
5. **Cross-page group alerts**: lobby channel `new_group_msg` broadcast → presenceStore handler → marks unread + plays sound (skipped if user is currently viewing that group)
6. **Session-end alerts**: `dm_ended` / `group_ended` lobby broadcasts → sound + re-fetch
7. **Friend request alert**: `friend_request` lobby broadcast → sound + re-fetch friends
8. **Nav badges**: Layout reads `unreadDMs.size` and `unreadGroups.size` for badge counts
9. **Mark as read**: `markRead()` called on chat mount → updates localStorage timestamp + removes from unread Set

### Files
- `src/lib/sounds.ts` — `playMessageSound`, `playNudgeSound`, `isSoundEnabled`, `setSoundEnabled`
- `src/stores/presenceStore.ts` — lobby broadcast listeners for all notification events
- `src/stores/dmStore.ts` — `unreadDMs`, `markRead`, unread checking in `fetchSessions`
- `src/stores/groupStore.ts` — `unreadGroups`, `markRead`, `checkUnread`
- `src/components/Layout.tsx` — sound toggle, nav badges

### PRD Coverage
NOTIF-1 through NOTIF-10 (§2.9). NOTIF-9 (push notifications) is 🔲 Future.

### Audit Status: PASS
- Sound functions have try-catch (AudioContext may not be available)
- Unread detection is localStorage-based — consistent within a single browser, not cross-device
...
- **Audit: PASS** — Note: only detects full browser offline, not Supabase Realtime channel drops (see PRD Gap #1 below)
...
|---|-------|---------|----------|--------|
| 1 | `onAuthStateChange` callback lacked try-catch | `authStore.ts` | Medium | ✅ Fixed |
| 2 | `acceptRequest`, `rejectRequest`, `removeFriend` lacked try-catch | `friendStore.ts` | Medium | ✅ Fixed |
| 3 | `fetchHistory` in both chat hooks lacked try-catch | `useChat.ts`, `useDMChat.ts` | Low-Med | ✅ Fixed |
...
| 8 | DM incoming messages don't appear in real-time — broadcast-only channels (no presence) unreliably deliver messages; no fallback mechanism to catch missed broadcasts | `useDMChat.ts` | High | ✅ Fixed |
...
|-------------|-------------------|--------|
| §2.1 Authentication | §1 | AUTH-1 through AUTH-8 |
| §2.2 Profile Management | §2 | PROF-1 through PROF-6 |
| §2.3 Friends | §4 | FRI-1 through FRI-13 |
| §2.4 Friend Categories | §5 | CAT-1 through CAT-8 |
| §2.5 Direct Messages | §6 | DM-1 through DM-14 |
| §2.6 Groups | §7 | GRP-1 through GRP-13 |
| §2.7 Chat (Real-time) | §7, §8 | CHAT-1 through CHAT-15 |
| §2.8 Presence | §3 | PRES-1 through PRES-7 |
| §2.9 Notifications | §9 | NOTIF-1 through NOTIF-10 |
| §2.10 Navigation & Layout | §11 | NAV-1 through NAV-15 |
...
|----|-----------|----------------------|----------|
| NAV-4 | "connection banner when real-time connection drops" | `ConnectionBanner.tsx` only detects full browser offline (`navigator.onLine`), not Supabase Realtime channel disconnection | Low — Supabase client has its own reconnection logic; banner is supplementary |
