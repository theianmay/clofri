...
### Core Principles
1. **Ephemeral by default** — Messages don't persist beyond the session
2. **Presence-first** — Always know who's around
3. **Low friction** — Magic link login, no app install, minimal setup
4. **Trust-based** — Friend codes, invite-only groups, no public discovery
...
|----|-----------|--------|-------|
| AUTH-1 | As a new user, I can sign up by entering my email and clicking a magic link sent to my inbox | ✅ Done | Supabase OTP via `signInWithMagicLink` |
| AUTH-2 | As a returning user, I can log in via the same magic link flow | ✅ Done | Session persists via Supabase auth state |
| AUTH-3 | As a logged-in user, I can log out from the sidebar | ✅ Done | Clears session, redirects to login |
...
| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| PROF-1 | As a user, I can see my display name, avatar, and friend code in the sidebar | ✅ Done | Layout sidebar profile section |
| PROF-2 | As a user, I can edit my display name inline in the sidebar | ✅ Done | Click pencil icon → inline input → saves to Supabase |
| PROF-3 | As a user, I can pick from a set of predefined icon avatars (cat, dog, etc.) | ✅ Done | AvatarPicker component, stored as `icon:name` in `avatar_url` |
| PROF-4 | As a user, I can copy my unique friend code to share with others | ✅ Done | Copy button with checkmark feedback |
| PROF-5 | As a user, I can upload a custom avatar image | 🔲 Future | Requires Supabase Storage integration |
| PROF-6 | As a user, I have a unique auto-generated friend code assigned on profile creation | ✅ Done | 8-char uppercase code generated by DB |
...
| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| FRI-1 | As a user, I can send a friend request by entering someone's friend code | ✅ Done | `sendRequest` in friendStore |
...
| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| CAT-1 | As a user, I can create a named category for organizing friends | ✅ Done | Auto-assigned color from palette |
| CAT-2 | As a user, I can assign a friend to a category | ✅ Done | Dropdown on each friend card |
| CAT-3 | As a user, I can filter the friends list to show only friends in a specific category | ✅ Done | Filter dropdown at top of friends list |
...
**Current state:** DMs use a dedicated `dm_sessions` table. Sessions are ephemeral — either user can end the conversation, which deletes all messages and closes the session for both sides.

| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| DM-1 | As a user, I can start a DM session with a friend from the Friends page | ✅ Done | `startSession` creates or reuses active dm_session |
| DM-2 | As a user, I can send and receive real-time messages in a DM | ✅ Done | Broadcast channel `dm-session:{id}` |
| DM-3 | As a user, I can see my friend's display name, avatar, and online status in the DM header | ✅ Done | Presence dot + status text |
| DM-4 | As a user, I can see typing indicators when my friend is typing | ✅ Done | "typing..." text in header |
| DM-5 | As a user, I can end a DM session, which deletes all messages and closes the chat for both sides | ✅ Done | XCircle button → confirm → `endSession` |
| DM-6 | As a user, when the other person ends the DM, I hear a notification sound and am redirected to Messages | ✅ Done | `dm_ended` lobby broadcast → sound + redirect |
| DM-7 | As a user, I can see a list of my active DM sessions on the Messages page | ✅ Done | Messages page shows active sessions with presence |
| DM-8 | As a user, I see an unread indicator on DM sessions that have new messages | ✅ Done | Blue dot on session card + Messages nav badge |
| DM-9 | As a user, I hear a notification sound when I receive a DM while on another page | ✅ Done | `new_dm` lobby broadcast → sound + unread |
...
**Current state:** Users can create groups, invite others via a 6-character code, and chat in real-time. Groups are ephemeral — the creator can end the session, which deletes messages and deactivates the group.

| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| GRP-1 | As a user, I can create a new group with a name | ✅ Done | `createGroup` generates invite code, adds creator as member with 'creator' role |
| GRP-2 | As a user, I can join a group by entering a 6-character invite code | ✅ Done | `joinGroupByCode` with duplicate handling |
| GRP-3 | As a user, I see an error if I enter an invalid code, already-joined group, or ended session | ✅ Done | Validation + is_active check |
...
| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| PRES-1 | As a user, I appear as "active" (green) when I'm using the app | ✅ Done | Tracked via lobby presence channel |
| PRES-2 | As a user, I transition to "idle" (amber) after 5 minutes of no mouse/keyboard activity | ✅ Done | `IDLE_TIMEOUT_MS = 5 * 60 * 1000` |
| PRES-3 | As a user, I transition to "idle" immediately when I switch to another tab | ✅ Done | `visibilitychange` event listener |
| PRES-4 | As a user, I return to "active" when I interact with the app again | ✅ Done | Mouse/keyboard/scroll event listeners |
| PRES-5 | As a user, I appear as "offline" (gray) when I close the app | ✅ Done | Channel unsubscribe + stale threshold |
| PRES-6 | As a user, I can see others' presence status on the Friends page, Messages page, and in chat headers | ✅ Done | Presence dots throughout the app |
| PRES-7 | As a user, my status should display correctly to others even if I don't interact for a while | ✅ Done | Heartbeat every 60s keeps `last_active` fresh; stale threshold at 6min |
...
| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| NOTIF-1 | As a user, I hear a notification sound when a message arrives in a group I have open | ✅ Done | `playMessageSound` in useChat |
| NOTIF-2 | As a user, I hear a notification sound when a DM arrives while I'm on any page | ✅ Done | `new_dm` lobby broadcast → presenceStore handler |
| NOTIF-3 | As a user, I can toggle notification sounds on/off from the sidebar | ✅ Done | Volume icon toggle, persisted in localStorage |
| NOTIF-4 | As a user, I see a blue dot on group cards that have unread messages | ✅ Done | `unreadGroups` Set + localStorage timestamps |
| NOTIF-5 | As a user, I see a blue dot on DM sessions that have unread messages | ✅ Done | `unreadDMs` Set + localStorage timestamps |
| NOTIF-6 | As a user, I see an unread count badge on the Messages nav item | ✅ Done | `unreadDMCount` in Layout |
...
| ID | User Story | Status | Notes |
|----|-----------|--------|-------|
| NAV-1 | As a user, I can navigate between Friends, Messages, and Groups via the sidebar | ✅ Done | NavLink with active state highlighting |
| NAV-2 | As a desktop user, I can collapse the sidebar to a narrow icon rail | ✅ Done | Toggle button, persisted in localStorage |
| NAV-3 | As a mobile user, I can open/close the sidebar via a hamburger menu | ✅ Done | Slide-out overlay, auto-closes on navigation |
...
- **groups** — Chat groups (name, creator, invite_code, is_active)
- **group_members** — Group membership (group_id, user_id, role)
- **messages** — Group messages (group_id, user_id, text)
- **dm_sessions** — DM session lifecycle (user1_id, user2_id, is_active)
- **direct_messages** — DM messages (sender_id, receiver_id, session_id, text)
...
- **Push notifications** — Web push for background alerts
- **Custom avatar uploads** — Supabase Storage integration
- **Media messages** — Image sharing in chat
- **Friend categories sync** — Migrate from localStorage to Supabase
- **Message reactions** — Emoji reactions on messages
- **Rate limiting** — Server-side message throttling

### Long-term
- **Mobile app** — React Native reusing same backend
- **Voice rooms** — WebRTC audio channels
- **E2E encryption** — Optional per-group encryption
- **Threads** — Reply threads within chats
